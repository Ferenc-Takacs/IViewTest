    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            if let Some(path) = start_image {
                ...
                app.open_image(&cc.egui_ctx, None, &path, !clipboard);
            } else {
                app.open_image_dialog(&cc.egui_ctx, None, &None);
            }
            Ok(Box::new(app))
        }),
    )
...
impl ImageViewer {
    ...
    fn copy_from_clipboard(&mut self, ctx: &egui::Context, frame: Option<&mut eframe::Frame>) {
        if let Some(temp_path) = save_clipboard_image() {
            self.image_full_path = Some(temp_path); // nem állunk rá a tmp könyvtárra
            self.load_image(ctx, frame, false);
        }
    }
    ...
    fn open_image(&mut self, ctx: &egui::Context, frame: Option<&mut eframe::Frame>, path: &PathBuf, make_list: bool) {
    ...
    fn open_image_dialog(&mut self, ctx: &egui::Context, frame: Option<&mut eframe::Frame>, def: &Option<PathBuf>) {
        let mut dialog = rfd::FileDialog::new()
            .add_filter(
                "Images",
        ...
        if let Some(path) = dialog.pick_file() {
            self.open_image(ctx, frame, &path, true);
        }
    }

    fn review(&mut self, ctx: &egui::Context, frame: Option<&mut eframe::Frame>, coloring: bool, new_rotate: bool) {
        if let Some(mut img) = self.original_image.clone() {
            if coloring {
            ...
            //self.texture = Some(ctx.load_texture("kep", color_image, Default::default())); // régi CPU
            //GPU
            if let Some(fram) = frame {
                self.review_gpu( ctx, fram, &color_image);
            }
            else {
    ...
    fn load_image(&mut self, ctx: &egui::Context, frame: Option<&mut eframe::Frame>, reopen: bool) {
        ...
        if let Ok(mut img) = image::open(&filepath) {
            ...
            self.review(ctx, frame, false, false);
        }
    }

    fn navigation(&mut self, ctx: &egui::Context, frame: Option<&mut eframe::Frame>,  irany: i32) {
        ...
        self.open_image(ctx, frame, &self.list_of_images[uj_index].path(), false);
    }
}
...
impl eframe::App for ImageViewer {
    ...
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        // Az update() függvény elején:
        if let Some(render_state) = frame.wgpu_render_state() {
            if self.image_processor.is_none() {
                // Létrehozzuk a processzort (ez csak egyszer fut le)
                let processor = gpu_processor::ImageProcessor::new(
                    &render_state.device,
                    &render_state.queue,
                ...
                self.image_processor = Some(processor_arc.clone());
                
                let mut renderer = render_state.renderer.write();
                // Ezzel tesszük elérhetővé a CallbackResources-en keresztül
                renderer.callback_resources.insert(processor_arc); 
                self.settings_dirty = true; // Kényszerítjük az első LUT feltöltést
            }
            
            if let Some(color_image) = self.pending_image.take() {
                self.review_gpu(ctx, frame,&color_image);
            }
        }

        
        if self.settings_dirty {
            if let (Some(processor), Some(render_state)) = (&self.image_processor, frame.wgpu_render_state()) {
                // 1. Legyártjuk az új 33x33x33-as tömböt a CPU-n
                let lut_data = gpu_processor::Lut3D::new(&self.color_settings);
                
                // 2. Feltöltjük a meglévő 3D textúrába
                processor.update_lut(&render_state.queue, &processor.lut_texture, &lut_data.data);
                
                // 3. Frissítjük a többi paramétert (Sharpness, Blur, stb.) a shadernek
                let params = gpu_processor::GpuParams::from_color_settings(&self.color_settings);
                render_state.queue.write_buffer(
                    &processor.params_buffer,
                    0,
        ...
        if ctx.input_mut(|i| {
            ...
            self.review(ctx, Some(frame), true, false);
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::COMMAND,
                egui::Key::G,
            ...
            self.review(ctx,Some(frame), true, true);
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::COMMAND,
                egui::Key::ArrowRight,
            ...
            self.review(ctx, Some(frame), true, r);
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::ALT,
                egui::Key::C,
            ...
            // paste
            // not work with Ctrl
            self.copy_from_clipboard(ctx, Some(frame));
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::NONE,
                egui::Key::C,
            ...
            // open
            self.open_image_dialog(ctx, Some(frame), &None);
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::NONE,
                egui::Key::R,
            ...
            // reopen
            self.load_image(ctx, Some(frame), true);
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::NONE,
                egui::Key::S,
            ...
            // next
            self.navigation(ctx, Some(frame), 1);
        } else if ctx.input_mut(|i| {
            i.consume_shortcut(&egui::KeyboardShortcut::new(
                egui::Modifiers::NONE,
                egui::Key::D,
            ...
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("Options", |ui| {
                    ...
                    ui.menu_button("Position", |ui| {
                        ...
                        if changed {
                            self.load_image(ctx, Some(frame), false);
                            ui.close_menu();
                        }
                    });
        ...
        if let Some(path) = dropped_file {
            self.open_image(ctx, Some(frame), &path.to_path_buf(), true);
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                if self.current_gpu_view.is_some() {
                    egui::Frame::canvas(ui.style())
                        ...
                        .show(ui, |ui| {
                            ...
                            if let Some(_processor) = &self.image_processor {
                                let callback = egui::PaintCallback {
                                    ...
                                    callback: std::sync::Arc::new(egui_wgpu::Callback::new_paint_callback(
                                     rect, ImageCallback {lut_weight: 1.0, })),
                                };

                                // 3. A callback hozzáadása a festőhöz
                            ...
                            let output = scroll_area.show(ui, |ui2| {
                                // Kiszámoljuk a méretet (new_size már megvan neked)
                                let (rect, _response) = ui2.allocate_at_least(new_size, egui::Sense::hover());

                                if let Some(processor) = &self.image_processor {
                                    let callback = egui::PaintCallback {
                                        rect,
                                        callback: std::sync::Arc::new(egui_wgpu::Callback::new_paint_callback(rect, gpu_processor::ImageCallback {
                                            lut_weight: 1.0,
                                        })),
                                    };
                                    ui2.painter().add(callback);
                                }
                            });
        ...
        if let Some((type_str, path)) = pending_action {
            match type_str {
                "OPEN" => self.open_image(ctx, Some(frame), &path, true),
                "OPEN_DIAL" => self.open_image_dialog(ctx, Some(frame), &Some(path)),
                "SAVE_DIAL" => {
                    self.save_original = true;
        ...
        if self.color_correction_dialog {
            ...
            egui::Window::new("Color corrections")
                ...
                ui.label(egui::RichText::new("Sharpen & Blur").strong());
                ui.horizontal(|ui| {
